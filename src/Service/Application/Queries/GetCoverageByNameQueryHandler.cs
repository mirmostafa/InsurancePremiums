...
internal sealed class GetCoverageByIdQueryHandler(InsurancePremiumsReadDbContext readDbContext, CancellationToken cancellationToken = default) : IQueryHandler<GetCoverageByIdQueryParams, GetCoverageByIdQueryResult>
{
    public async Task<GetCoverageByIdQueryResult> HandleAsync(GetCoverageByIdQueryParams query)
    {
        var dbQuery = from coverage in readDbContext.Coverages
        ...
        if (dbResult == null)
        {
            return new(Result<CoverageDto>.CreateFailure(new NoItemFoundException("No coverage found with the specific Id.")));
        }
        var result = dbResult.ToDto();
        return new(Result.CreateSuccess(result));
    }
